...
def compute_correlation_analysis(networks: List[Dict]) -> Dict:
    ...
    if len(valid_networks) < 10:
        print(
            "Warning: Not enough networks with complete metrics for correlation analysis"
        )
        return correlations
    ...
    for metric in metrics_to_analyze:
        ...
        valid_pairs = [
            (v, m) for v, m in zip(values, mapes) if v is not None and m is not None
        ]
...
def analyze_generalization_gaps(
    ...
    if topology_zoo_dir:
        ...
        def compute_graph_metrics(G: nx.Graph) -> Dict:
            ...
            metrics["density"] = (
                G.number_of_edges() / (n * (n - 1) / 2.0) if n > 1 else 0.0
            )
            ...
            if n <= 100:
                ...
                sample_nodes = np.random.choice(
                    list(G.nodes()), sample_size, replace=False
                )
    ...
    for metric, corr_data in sorted(
        correlations.items(), key=lambda x: abs(x[1]["pearson_r"]), reverse=True
    ):
        ...
        significance = (
            "***"
            if pearson_p < 0.001
            else "**" if pearson_p < 0.01 else "*" if pearson_p < 0.05 else ""
        )

        print(
            f"{metric:<25} {pearson_r:>10.3f}{significance:<5} {pearson_p:>10.3e} {spearman_r:>10.3f} {n:>10}"
        )
    ...
    for metric in comparison_metrics:
        ...
        if top_val is not None and bottom_val is not None:
            ...
            print(
                f"{metric:<25} {top_val:>15.3f} {bottom_val:>15.3f} {diff:>12.3f} ({diff_pct:>6.1f}%)"
            )
    ...
    if significant_correlations:
        ...
        for corr in significant_correlations:
            ...
            print(
                f"  • {corr['metric']}: {direction} MAPE (r={corr['pearson_r']:.3f}, p={corr['pearson_p']:.3e})"
            )
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Analyze generalization gaps in Topology Zoo results"
    )
    parser.add_argument(
        "--results_file",
        type=str,
    ...
    analyze_generalization_gaps(
        args.results_file, args.topology_zoo_dir, args.output_file
    )
